...
//        myRobot.setCraterMode(Math.abs(angleToGo) < Math.toRadians(50));
    ...
    public void update(){
        ...
        boolean isAllowedToFeed = myRobot.getYPos() < Field.FIELD_LENGTH*0.6 || isAuto;
        ...
        if(allowedToDrive && isAllowedToFeed){
            if(myState >= myStates.retractingCollectorAndMoveInLift.ordinal() &&
                ...
                if(isAuto && (rollerReverseTime == 0 ||
                    ...
                    MovementVars.movement_turn *= 0.25;
                }
            }
        }
        ...
        double currRollerMaxPower = myCollector.getExtensionPercent() > -0.03 ? -0.8 : -0.4;
        ...
        if(myState == myStates.retractingCollectorAndMoveInLift.ordinal()){
            ...
            double collectorWithPrediction = myCollector.getExtensionPercent() +
                    myCollector.getExtensionCurrentSpeedPercent() * 0.015;//0.05
...
//                myLift.setCurrentPositionTicks(0);
            ...
            boolean activateRoller = collectorWithPrediction < 0.06
...
//            myCollector.reverseCollector();
            ...
            if(currTimeMillis > rollerReverseTime + 375){
                if(isAllowedToFeed){
                    nextStage();
                }else{
                    abortAutoFeed();
...
//            myLift.dump();
            ...
            //release the minerals when 275 millis has passed
            if(currTimeMillis - stateStartTime > 275){
                myLift.releaseMinerals();
            }
